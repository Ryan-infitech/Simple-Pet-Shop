...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (startDate) {
      whereClause += " AND a.booking_date >= ?";
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    result.data.forEach((appointment) => {
      const date = appointment.booking_date.toISOString().split("T")[0];
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const conflictResult = await executeQuery(
      'SELECT id FROM service_bookings WHERE service_id = ? AND booking_date = ? AND booking_time = ? AND status IN ("scheduled", "confirmed")',
    ...
    const appointmentDateTime = new Date(`${booking_date} ${booking_time}`);
    const now = new Date();
    ...
    // Create appointment
    const result = await executeQuery(
      `INSERT INTO appointments (user_id, service_id, booking_date, booking_time, 
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const appointmentResult = await executeQuery(
      `SELECT * FROM service_bookings ${whereClause}`,
      checkParams
    );
    ...
    if (booking_date && booking_time) {
      const conflictResult = await executeQuery(
        'SELECT id FROM service_bookings WHERE service_id = ? AND booking_date = ? AND booking_time = ? AND status IN ("scheduled", "confirmed") AND id != ?',
        [appointment.service_id, booking_date, booking_time, appointmentId]
      );
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const appointmentExists = await executeQuery(
      "SELECT id, status FROM service_bookings WHERE id = ?",
      [appointmentId]
    );
